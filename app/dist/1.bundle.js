...
/* harmony import */ var socket_io_client__WEBPACK_IMPORTED_MODULE_4__ = __webpack_require__(23);
...
var promisify = function promisify(f) {
  return function () {
    ...
    return new Promise(function (resolve, reject) {
      ...
      f.call.apply(f, [_this].concat(args, [callback]));
    });
  };
};

var socket = socket_io_client__WEBPACK_IMPORTED_MODULE_4___default()();
var on = promisify(socket.on); // plan:
// 1. simpolify my work by adding interface showing all players that are in game
// 2. separate client-server and client parts
// 3. make game interface nearly true
// 4. JUST FRICKING DO IT ,sldkfkpdfg,sefkpD,c!
...
var CheckersGame = /*#__PURE__*/function (_React$Component3) {
  ...
  function CheckersGame(props) {
    ...
    _this3.handleCheckersClick = _this3.handleCheckersClick.bind(_assertThisInitialized(_this3));
    _this3.history = [];
    /*let playersURL = new URL(document.location + '/players');
    // playersURL.searchParams.set('players', '')
    let players = fetch(playersURL)
    			.then(response => {
