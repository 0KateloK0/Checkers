...
function CheckerField (props) {
	...
	return (
		<div className={'checker-field checker-field-' + props.bg} onClick={props.onClick} pos={props.pos}>
			{checker}
		</div>
		)
}
...
class Checkers extends React.Component {
	...
	handleClick (e) {
		let [row, col] = [...e.target.closest('.checker-field').getAttribute('pos').split('_')];
		let place = this.state.field[row][col]; // new place
		let field = [].concat(this.state.field); // just a copy
			...
			<div className="field">
				{this.state.field.reduce((arr, el, i) => 
					arr.concat(el.map((checker, j) => 
						<CheckerField key={i + '_' + j} pos={i + '_' + j} checker={checker} checked={checked(i, j)} 
